...
( function() {
	CKEDITOR.plugins.add( 'anchor', {
		...
		icons: 'anchor,anchor-rtl', // %REMOVE_LINE_CORE%
		...
		init: function( editor ) {
			...
			if ( editor.ui.addButton ) {
				editor.ui.addButton( 'Anchor', {
					label: editor.lang.anchor.anchor.toolbar,
					command: 'anchor',
					toolbar: 'links,30'
				} );
			}
			...
			editor.on( 'doubleclick', function( evt ) {
				...
				if ( element && !element.isReadOnly() ) {
					if ( element.is( 'a' ) && element.getAttribute( 'name' ) && !element.getAttribute( 'href' ) && !element.getChildCount() ) {
						evt.data.dialog = ( element.getAttribute( 'name' ) && ( !element.getAttribute( 'href' ) || !element.getChildCount() ) ) ? 'anchor' : 'link';

						// Pass the link to be selected along with event data.
						evt.data.link = element;
					} else if ( CKEDITOR.plugins.anchor.tryRestoreFakeAnchor( editor, element ) ) {
			...
			if ( editor.addMenuItems ) {
				editor.addMenuItems( {
					anchor: {
						label: editor.lang.anchor.anchor.menu,
						command: 'anchor',
						group: 'anchor',
						order: 1
					},
					...
					removeAnchor: {
						label: editor.lang.anchor.anchor.remove,
			...
			if ( editor.contextMenu ) {
				editor.contextMenu.addListener( function( element ) {
					...
					var anchor = CKEDITOR.plugins.anchor.tryRestoreFakeAnchor( editor, element );

					if ( !anchor && !( anchor = CKEDITOR.plugins.anchor.getSelectedLink( editor ) ) ) {
						return null;
					}
		...
		afterInit: function( editor ) {
			...
			if ( pathFilters ) {
				pathFilters.push( function( element, name ) {
					if ( name == 'a' ) {
						if ( CKEDITOR.plugins.anchor.tryRestoreFakeAnchor( editor, element ) || ( element.getAttribute( 'name' ) && ( !element.getAttribute( 'href' ) || !element.getChildCount() ) ) ) {
							return 'anchor';
						}
					}
				} );
	...
	CKEDITOR.removeAnchorCommand.prototype = {
		exec: function( editor ) {
			...
			if ( sel && ( anchor = sel.getSelectedElement() ) && ( !anchor.getChildCount() ? CKEDITOR.plugins.anchor.tryRestoreFakeAnchor( editor, anchor ) : anchor.is( 'a' ) ) ) {
				anchor.remove( 1 );
			} else {
				if ( ( anchor = CKEDITOR.plugins.anchor.getSelectedLink( editor ) ) ) {
